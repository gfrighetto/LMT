clc, clear, daqreset; delete(imaqfind); % clean the Command Window, Workspace, DAQ objects

%% CAMERA INITIALIZATION:

imaqhwinfo
% Set up camera for vidData acquisition
vid = videoinput('mwspinnakerimaq', 1, 'Mono8');
vid.FramesPerTrigger = Inf;
vid.ROIPosition = [148 60 420 420];                                            % Set your ROI and pixel resolution frames captured [X-Offset Y-Offset Width Height]
triggerconfig(vid, 'manual');
src = getselectedsource(vid);
src.AcquisitionFrameRate = 100;                                             % Use the camera in Mono8 mode to acquire 8-bit data, smaller videos
src.Gain = 40;
open src
%vid.LoggingMode = 'memory';                                                 % data logged to camera memory
...
% start(vid)
% trigger(vid)
% pause(201) % pause of 201s
% stop(vid)

%% INITIALIZE VIDEOPLAYER AND GET SNAPSHOT:


videoPlayer = vision.VideoPlayer('Position',  [0, 0, 420, 420]);
...
% imshow(imadjust(getsnapshot(vid)));
...
Video = getsnapshot(vid);
[w,h] = size(Video(:,:,1,1));

% initialize dataframe and frame counting:
Track={};
i=1;
...
VideoFrame = getsnapshot(vid); % get a snaphot
I = VideoFrame(:,:,1,:);       % extract a frame from the video
...
[max, ind] = maxk(FPSelect.Scale(:),1);
PointSelect = FPSelect(ind);
...
%% GET HEAD ANGLE from SURFFeatures() detection
...
%     'LineWidth',2,...
%    'Color', 'g')
% Plot fly orientation based on the head position:
VideoFrame = insertShape(VideoFrame,'Line',[x1 y1 x2 y2],'LineWidth',1, 'Color', {'green'});
...
% Plot fly orientation based on the body orientation:
VideoFrame = insertShape(VideoFrame,'Line',[x1 y1 x2 y2],'LineWidth',5, 'Color', {'blue'});

% Plot head orientation:
VideoFrame = insertShape(VideoFrame,'Line',[PointSelect.Location(1,1) PointSelect.Location(1,2) PointSelect.Location(2,1) PointSelect.Location(2,2)],'LineWidth',1, 'Color', {'green'});
...
step(videoPlayer, VideoFrame); % show video with point tracking.

end
toc

bodyangle = vertcat(Track.Bangle);
plot(bodyangle, 'b')
hold on
headangle = vertcat(Track.Hangle);
plot(headangle, 'g')
